...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const crossPiont = x1 * x2 + y1 * y2;
  return Math.acos(crossPiont / (Math.hypot(x1, y1) * Math.hypot(x2, y2)));
}
...
function roundToPowerOfTen(num, pow) {
  return Math.round(num / (10 ** pow)) * 10 ** pow; // 10 is magic number
}
...
function toNumber(v, d) {
  ...
  return Number.isNaN(number) ? d : number;
}
