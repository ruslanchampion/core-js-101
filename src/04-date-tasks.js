...
function isLeapYear(date) {
  ...
  if (currentYear % 4) return false;
  if (currentYear % 100) return true;
  if (currentYear % 400) return false;
  return true;
}
...
function angleBetweenClockHands(date) {
  const angleRad = Math.abs(0.5 * (60 * (date.getUTCHours() % 12) - 11 * date.getUTCMinutes()));
  return ((angleRad > 180 ? 360 - angleRad : angleRad) * Math.PI) / 180;
}
